...
function safePatternId(raw) {
  return raw.replace(
    /[^a-zA-Z0-9_-]/g,
    (ch) => `_${ch.charCodeAt(0).toString(16).padStart(2, "0")}`
  );
}
...
function createCentralSpringForce(axis, target, constantMagnitude) {
  let nodes = [];
...
function createForceGraph(data, opts = {}) {
  ...
  const {
    ...
    nodeStrokeOpacity = 1,
    nodeStrength,
    linkSource = (link2) => link2.source,
    linkTarget = (link2) => link2.target,
  ...
  let nodeFill = initialNodeFill;
  let nodeStroke = initialNodeStroke;
  let nodeStrokeWidth = initialNodeStrokeWidth;
  let linkStroke = initialLinkStroke;
  let currentNodes = [...initialNodes];
  let currentLinks = [...initialLinks];
  let nodeIds = currentNodes.map(
    (node, index2) => String(intern(nodeId(node, index2)))
  );
  let nodeTitles = nodeTitle ? currentNodes.map(
    (node, index2) => nodeTitle(node, index2)
  ) : null;
  let radiusValues = typeof nodeRadius === "function" ? currentNodes.map(
    (node, index2) => nodeRadius(node, index2)
  ) : null;
  let nodeFillValues = typeof nodeFill === "function" ? currentNodes.map(
    (node, index2) => nodeFill(node, index2)
  ) : null;
  let nodeStrokeValues = typeof nodeStroke === "function" ? currentNodes.map(
    (node, index2) => nodeStroke(node, index2)
  ) : null;
  let nodeStrokeWidthValues = typeof nodeStrokeWidth === "function" ? currentNodes.map(
    (node, index2) => nodeStrokeWidth(node, index2)
  ) : null;
  let linkSources = currentLinks.map(
    (link2, index2) => String(intern(linkSource(link2, index2)))
  );
  let linkTargets = currentLinks.map(
    (link2, index2) => String(intern(linkTarget(link2, index2)))
  );
  let linkStrokeWidths = typeof linkStrokeWidth === "function" ? map(currentLinks, linkStrokeWidth) : null;
  let linkStrokeColors = typeof linkStroke === "function" ? map(currentLinks, linkStroke) : null;
  let forceNodes = currentNodes.map(
    (node, index2) => ({
      ...node,
      id: String(nodeIds[index2]),
      __index: index2
    })
  );
  let forceLinks = currentLinks.map(
    (link2, index2) => ({
      ...link2,
      source: linkSources[index2],
      target: linkTargets[index2]
    })
  );
  const getNodeRadius = (_d, index2) => {
    if (typeof nodeRadius === "number") {
      return nodeRadius;
    }
    return radiusValues ? radiusValues[index2] : 5;
  };
  const IMAGE_CORNER_RADIUS = 6;
  const applyNodeSize = (sel) => {
    sel.attr("width", (d, i) => getNodeRadius(d, i) * 2).attr("height", (d, i) => getNodeRadius(d, i) * 2).attr(
      "rx",
      (d, i) => d.thumbnailUrl ? IMAGE_CORNER_RADIUS : getNodeRadius(d, i)
    ).attr(
      "ry",
      (d, i) => d.thumbnailUrl ? IMAGE_CORNER_RADIUS : getNodeRadius(d, i)
    );
  ...
  let currentTheme = {
    nodeFill: typeof nodeFill === "string" ? nodeFill : void 0,
    nodeStroke,
    nodeStrokeOpacity,
    nodeStrokeWidth,
    linkStroke: typeof linkStroke === "string" ? linkStroke : void 0,
    linkStrokeOpacity
  };
  const getNodeFill = (d, index2) => {
    if (typeof nodeFill === "string") {
      return nodeFill;
    }
    if (typeof nodeFill === "function") {
      return currentTheme.nodeFill || (nodeFillValues ? nodeFillValues[index2] : "currentColor");
    }
    return nodeFillValues ? nodeFillValues[index2] : "currentColor";
  };
  const getLinkStrokeWidth = (_d, index2) => {
    if (typeof linkStrokeWidth === "number") {
      return linkStrokeWidth;
    }
    return linkStrokeWidths ? linkStrokeWidths[index2] : 1.5;
  };
  const getLinkStrokeColor = (_d, index2) => {
    if (typeof linkStroke === "string") {
      return linkStroke;
    }
    return linkStrokeColors ? linkStrokeColors[index2] : "#999";
  };
  const linkForce = link(forceLinks).id((node) => node.id);
  ...
  const simulation$1 = simulation(forceNodes).alphaDecay(2e-3).force("link", linkForce).force("charge", nodeForce).on("tick", ticked);
  if (centerStrengthX !== void 0 || centerStrengthY !== void 0 || centerX !== void 0 || centerY !== void 0) {
    ...
    simulation$1.force("centerX", piecewiseX).force("centerY", piecewiseY);
  } else {
    simulation$1.force("center", center());
  }
  let currentWidth = Math.max(1, width);
  let currentHeight = Math.max(1, height);
  const svg = create$1("svg").attr("width", currentWidth).attr("height", currentHeight).attr("viewBox", [
    -currentWidth / 2,
    -currentHeight / 2,
    currentWidth,
    currentHeight
  ...
  const tooltipPositioning = tooltipHost === document.body ? "fixed" : "absolute";
  ...
  const showTooltip = (event, node) => {
    const nodeTitleText = nodeTitles?.[node.__index] ?? node.title ?? "";
    if (!nodeTitleText) {
      return;
    }
  ...
  const positionTooltip = (event) => {
    ...
    if (tooltipPositioning === "fixed") {
      const x22 = event.clientX + offsetX;
      const y22 = event.clientY + offsetY;
      tooltip.style.transform = `translate(${x22}px, ${y22}px)`;
      return;
    }
    const rootRect = tooltipHost.getBoundingClientRect();
    const x2 = event.clientX - rootRect.left + offsetX;
    const y2 = event.clientY - rootRect.top + offsetY;
    tooltip.style.transform = `translate(${x2}px, ${y2}px)`;
  };
  const handlePointerEnter = (event, node) => {
  ...
  function syncThumbnailPatterns(nodes2) {
    const imageNodes = nodes2.filter((n) => n.thumbnailUrl);
    defs.selectAll(
      "pattern.node-thumb"
    ).data(imageNodes, (d) => d.id).join(
      (enter) => {
        const pattern = enter.append("pattern").attr("class", "node-thumb").attr(
          "id",
          (d) => `thumb-${safePatternId(d.id)}`
        ).attr("width", 1).attr("height", 1).attr(
          "patternContentUnits",
          "objectBoundingBox"
        );
        pattern.append("image").attr("href", (d) => d.thumbnailUrl).attr("width", 1).attr("height", 1).attr(
          "preserveAspectRatio",
          "xMidYMid slice"
        );
        return pattern;
      ...
      (update) => {
        update.select("image").attr("href", (d) => d.thumbnailUrl);
        return update;
      }
    );
  }
  const zoomLayer = svg.append("g").attr("data-graph-layer", "zoomable");
  const linkLayer = zoomLayer.append("g").attr(
    "stroke",
    typeof linkStroke === "string" ? linkStroke : null
  ).attr("stroke-opacity", linkStrokeOpacity).attr(
    "stroke-width",
    typeof linkStrokeWidth === "number" ? linkStrokeWidth : null
  ).attr("stroke-linecap", linkStrokeLinecap);
  const nodeLayer = zoomLayer.append("g").attr(
    "fill",
    typeof nodeFill === "string" ? nodeFill : null
  ).attr(
    "stroke",
    typeof nodeStroke === "string" ? nodeStroke : null
  ).attr("stroke-opacity", nodeStrokeOpacity).attr(
    "stroke-width",
    typeof nodeStrokeWidth === "number" ? nodeStrokeWidth : null
  );
  let linkSelection = linkLayer.selectAll("line").data(forceLinks).join("line");
  if (typeof linkStrokeDasharray === "function") {
    linkSelection.attr(
      "stroke-dasharray",
      (d, i) => linkStrokeDasharray(d, i)
    );
  ...
  let nodeSelection = nodeLayer.selectAll("rect").data(forceNodes).join("rect");
  ...
  nodeSelection.call(drag$1(simulation$1));
  const bindNodeInteractions = (selection2) => {
    selection2.on(
      "pointerenter.tooltip",
      (event, node) => handlePointerEnter(
        event,
        node
      )
    ).on(
      "pointermove.tooltip",
      (event) => handlePointerMove(event)
    ).on(
      "pointerleave.tooltip",
      () => handlePointerLeave()
    );
    if (onNodeClick) {
      selection2.on("click", (event, node) => {
      ...
      selection2.on("click", null);
    }
    return selection2;
  };
  bindNodeInteractions(nodeSelection);
  const initialZoomScale = 0.333;
  let currentTransform = identity.scale(initialZoomScale);
  const zoomBehavior = zoom().scaleExtent([0.1, 1]).filter((event) => {
    if (event.type === "wheel") {
  ...
  const applyTheme = (nextTheme) => {
    ...
    if (currentTheme.nodeFill && typeof nodeFill !== "function") {
    ...
    if (currentTheme.nodeStrokeOpacity !== void 0) {
      nodeSelection.attr(
        "stroke-opacity",
        currentTheme.nodeStrokeOpacity
      );
    }
    if (currentTheme.nodeStrokeWidth !== void 0 && typeof nodeStrokeWidth !== "function") {
      nodeSelection.attr(
        "stroke-width",
        currentTheme.nodeStrokeWidth
      );
    }
    if (currentTheme.linkStrokeOpacity !== void 0) {
      linkSelection.attr(
        "stroke-opacity",
    ...
    if (typeof linkStroke === "function") {
      linkStrokeColors = map(currentLinks, linkStroke);
  ...
  function getPosition(value, axis) {
    if (typeof value === "string") return 0;
    const coordinate = axis === "x" ? value.x : value.y;
    return coordinate ?? 0;
  }
  function ticked() {
    linkSelection.attr(
      "x1",
      (d) => getPosition(d.source, "x")
    ).attr(
      "y1",
      (d) => getPosition(d.source, "y")
    ).attr(
      "x2",
      (d) => getPosition(d.target, "x")
    ).attr(
      "y2",
      (d) => getPosition(d.target, "y")
    );
    nodeSelection.attr("x", (d) => (d.x ?? 0) - getNodeRadius(d, d.__index)).attr("y", (d) => (d.y ?? 0) - getNodeRadius(d, d.__index));
  }
  const updateData = (nextData) => {
    const nextNodes = Array.isArray(nextData?.nodes) ? nextData.nodes : [];
    const nextLinks = Array.isArray(nextData?.links) ? nextData.links : [];
    const previousNodeIds = new Set(
      forceNodes.map((node) => node.id)
    );
    const previousLinkKeys = new Set(
      forceLinks.map((link2) => getLinkKey(link2))
    );
    ...
    const existingLinkByKey = new Map(
      forceLinks.map(
        (link2) => [getLinkKey(link2), link2]
      )
    );
    currentNodes = [...nextNodes];
    currentLinks = [...nextLinks];
    nodeIds = currentNodes.map(
      (node, index2) => String(intern(nodeId(node, index2)))
    );
    const nextNodeIdSet = new Set(nodeIds);
    nodeTitles = nodeTitle ? currentNodes.map(
      (node, index2) => nodeTitle(node, index2)
    ) : null;
    radiusValues = typeof nodeRadius === "function" ? currentNodes.map(
      (node, index2) => nodeRadius(node, index2)
    ) : null;
    nodeFillValues = typeof nodeFill === "function" ? currentNodes.map(
      (node, index2) => nodeFill(node, index2)
    ) : null;
    nodeStrokeValues = typeof nodeStroke === "function" ? currentNodes.map(
      (node, index2) => nodeStroke(node, index2)
    ) : null;
    nodeStrokeWidthValues = typeof nodeStrokeWidth === "function" ? currentNodes.map(
      (node, index2) => nodeStrokeWidth(node, index2)
    ) : null;
    let nodesChanged = currentNodes.length !== forceNodes.length;
    let nodePropsChanged = false;
    const nextForceNodes = currentNodes.map((node, index2) => {
      const id2 = String(nodeIds[index2]);
      const existing = existingNodeById.get(id2);
      if (existing) {
        if (existing.title !== node.title || existing.weight !== node.weight || existing.thumbnailUrl !== node.thumbnailUrl) {
          nodePropsChanged = true;
        }
        existing.__index = index2;
    ...
    linkSources = currentLinks.map(
      (link2, index2) => String(intern(linkSource(link2, index2)))
    );
    linkTargets = currentLinks.map(
      (link2, index2) => String(intern(linkTarget(link2, index2)))
    );
    const nextLinkKeys = currentLinks.map(
      (_link, index2) => `${linkSources[index2]}->${linkTargets[index2]}`
    );
    const nextLinkKeySet = new Set(nextLinkKeys);
    linkStrokeWidths = typeof linkStrokeWidth === "function" ? map(currentLinks, linkStrokeWidth) : null;
    linkStrokeColors = typeof linkStroke === "function" ? map(currentLinks, linkStroke) : null;
    let linksChanged = currentLinks.length !== forceLinks.length;
    const nextForceLinks = currentLinks.map((link2, index2) => {
      const sourceId = linkSources[index2];
      const targetId = linkTargets[index2];
      const key = nextLinkKeys[index2];
    ...
    linkSelection = linkLayer.selectAll("line").data(forceLinks, getLinkKey).join(
    ...
    const joinedNodes = nodeLayer.selectAll("rect").data(forceNodes, (node) => node.id).join(
      (enter) => {
        const enterSelection = enter.append("rect");
      ...
      (exit) => {
        exit.on("pointerenter.tooltip", null).on("pointermove.tooltip", null).on("pointerleave.tooltip", null).on("click", null).remove();
        return exit;
      }
    );
    ...
    simulation$1.nodes(forceNodes);
    linkForce.links(forceLinks);
    if (nodesChanged || linksChanged || nodePropsChanged) {
      const targetAlpha = nodesChanged || linksChanged ? 0.45 : 0.25;
      simulation$1.alpha(
        Math.max(simulation$1.alpha(), targetAlpha)
      ).restart();
    }
    applyTheme({});
  };
  ...
  return {
    ...
    resize: (nextWidth, nextHeight) => {
      ...
      if (widthValue === currentWidth && heightValue === currentHeight) {
        return;
      }
      currentWidth = widthValue;
      currentHeight = heightValue;
      svg.attr("width", currentWidth).attr("height", currentHeight).attr("viewBox", [
        -currentWidth / 2,
        -currentHeight / 2,
        currentWidth,
        currentHeight
    ...
    updateNodeFill: (fn) => {
      nodeFill = fn;
      nodeFillValues = currentNodes.map(
        (node, i) => fn(node, i)
      );
      nodeSelection.attr("fill", (d, i) => {
        if (d.thumbnailUrl) {
    ...
    updateNodeStroke: (fn) => {
      nodeStroke = fn;
      nodeStrokeValues = currentNodes.map(
        (node, i) => fn(node, i)
      );
      nodeSelection.attr(
        "stroke",
    ...
    updateNodeStrokeWidth: (fn) => {
      nodeStrokeWidth = fn;
      nodeStrokeWidthValues = currentNodes.map(
        (node, i) => fn(node, i)
      );
      nodeSelection.attr(
        "stroke-width",
    ...
    updateLinkStroke: (fn) => {
      linkStroke = fn;
      linkStrokeColors = map(currentLinks, fn);
      linkSelection.attr(
        "stroke",
    ...
    resetView: () => {
      const baseTransform = identity.scale(initialZoomScale);
    ...
    refreshLinkStyles: () => {
      ...
      if (typeof linkStrength === "function") {
        linkForce.strength(linkStrength);
        if (simulation$1.alpha() < 0.05) {
          simulation$1.alpha(0.05).restart();
        }
      }
    },
    destroy: () => {
...
const ForceDirectedGraph = reactExports.forwardRef(({ data, width, height, darkMode, nodeFill, nodeStroke, nodeStrokeWidth, nodeRadius, linkStroke, linkStrokeDasharray, linkOpacity, linkStrength, onNodeClick }, ref) => {
  const containerRef = reactExports.useRef(null);
  const graphRef = reactExports.useRef(null);
  const onNodeClickRef = reactExports.useRef(onNodeClick);
  const themeRef = reactExports.useRef(getThemeOptions(!!darkMode));
  ...
  reactExports.useEffect(() => {
    ...
    if (!graphRef.current) {
      ...
      const handle = createForceGraph(data, {
        ...
        ...linkOpacity !== void 0 && { linkOpacity },
        nodeStrength: -50,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { position: "relative", width: "100%", height: "100%" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: containerRef, style: { width: "100%", height: "100%" } }) });
});
ForceDirectedGraph.displayName = "ForceDirectedGraph";
export {
  ForceDirectedGraph as F
};
